...
#if !defined(LINT)
static char rscid[] =
"$Id: ufs_args.c,v 1.1 2013/08/12 21:15:03 zma0472 Exp $";
#endif /*  !defined(LINT)  */

char *tstpath = UFS_DEFAULT_TSTPATH;
char *pidpath = UFS_DEFAULT_PIDPATH;
char *errpath = UFS_DEFAULT_ERRPATH;
char *logpath = UFS_DEFAULT_LOGPATH;
         ...
         case 'd': delay = strtoul(optarg, &endptr, 0);
                   ...
                   if (ULONG_MAX == delay) {
                       ...
                       return (-1);
                               ...
                               ('\0' ==  *endptr) ) {
                       if (delay < UFS_DELAY_MIN) {
                           ERR("UFS delay=%d < UFS_DELAY_MIN=%d",
                                                         delay, UFS_DELAY_MIN);
                           DBG("return (%d)", -1);
                           return (-1);
                       } else {
